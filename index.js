...
exports.initialize = function (_mongoose) {
  mongoose = _mongoose;
  try {
    ...
    if (ex.name === 'MissingSchemaError') {
      ...
      mongoose.model('IdentityCounter', counterSchema);
    }
    else
      throw ex;
  }
...
exports.plugin = function (schema, options) {
  ...
  if (!counterSchema) throw new Error("mongoose-auto-increment has not been initialized");

  var initializeDB = function(connection, callback) {
  ...
  schema.pre('save', function (next) {
    ...
    for(var i=0; i<mongoose.connections;i++) {
      if(mongoose.connections[i].name = this.db.name) {
        connection = mongoose.connections[i];
        break;
      }
    }

    initializeDB(connection, function(connection){
      var IdentityCounter = connection.model('IdentityCounter');
      var doc = context;
